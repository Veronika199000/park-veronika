...
#данные ближайших метеостаций к парку Зёлная роща в г.Екатеринбург
Yekaterinburg = data.frame(id="SVX",
                    latitude = c(56.82067),
                    longitude= c(60.594831))

station_list = meteo_nearby_stations(lat_lon_df = Yekaterinburg, 
...
station_list = station_list %>% filter(name %in% c("EKATERINBURG")) 


#Чтобы получить все данные с метеостации, нужно 
one_station = meteo_tidy_ghcnd("RSM00028440") 
...
Yekaterinburg_cum = all_data %>% mutate(month = month(date)) %>% 
  filter(month > 4 & month < 10) %>% 
  group_by(year) %>% 
  mutate(prcp_cum = cumsum(prcp))

Yekaterinburg_cum %>% summarise(prcp_avg = max(prcp_cum), n = n())
...
park_sf <- read_sf(str_interp("${getwd()}/Green.geojson"))
...
dates = MODISTools::mt_dates(product = "MOD16A2", lat = 56.820678, lon = 60.594831)
...
Yekaterinburg_ET =  MODISTools::mt_subset(product = "MOD16A2",
                                      lat = 56.820678,
                                      lon = 60.594831,
                                      band = "ET_500m",
                                      start = "2017-05-01", 
...
# 
Yekaterinburg_ET = Yekaterinburg_ET %>% filter(value < 32700) %>% dplyr::select(units,calendar_date,value) %>%
...
ggplot(Yekaterinburg_ET, aes(x=doy,y=ET))+
  geom_point()+
  geom_smooth()+
...
Prcp_cum = Yekaterinburg_cum %>% filter(year == 2017) %>% mutate(doy = yday(date)) %>% 
...
curve = loess(ET ~ doy, Yekaterinburg_ET) # модель 
...
ggplot(Prcp_cum, aes(x = doy,y = ETcum))+
  ...
  ylab("ET vs Precipitation,m3 for Yekaterinburg park, 2017")+
  theme_bw()
# Необходимый полив - большую часть времени полив не нужен
ggplot(Prcp_cum, aes(x = doy,y = irrigation*1000))+
  geom_line( color="red")+
  geom_hline(yintercept = 0)+
  ylab("Irrigation needed,l/m2 for Yekaterinburg park, 2017")+
  theme_bw()
